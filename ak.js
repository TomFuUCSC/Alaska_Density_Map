...
d3.json("ak-topo.json").then(function (alaska) {
    console.log(alaska);
    console.log(topojson.feature(alaska, alaska.objects.tracts).features)

    // map the topography of the state with the color scale
    svg.append("g")
        .selectAll("path")
        .data(topojson.feature(alaska, alaska.objects.tracts).features)
        .enter().append("path")
        .attr("fill", function (d) {
    ...
    svg.append("path")
        .datum(topojson.feature(alaska, alaska.objects.counties))
        .attr("fill", "none")
        .attr("stroke", "#000")
